create   view [blobTransInc].[SAB_ConsCSRequest] as select 
DML_Action,Start_LSN,Seq_Val,Instance,[CloseDate]=cast(case when [CloseDate] < cast('1901-01-01' as date) then null else [CloseDate] end as date)
,[CloseReason]
,[ContactMethod]
,[ContactParty]
,[CREATEDBY]
...
,[Type]=(select top 1 e.Enum from dbo.Enum e where e.EnumSet='SAB_ConsCSRequestType' and e.EnumCode=t.[Type]) ,TypeEnumID=[Type]
 from stage.SAB_ConsCSRequest t
GO
