create   view [blobTransFull].[SAB_ConsAttribute] as select 
LSN,[AttributeClass_BuildingType]=(select top 1 e.Enum from dbo.Enum e where e.EnumSet='SAB_ConsAttributeClass_BuildingType' and e.EnumCode=t.[AttributeClass_BuildingType]) ,AttributeClass_BuildingTypeEnumID=[AttributeClass_BuildingType]
...
,[UseAdjustedMeasurement]=case when [UseAdjustedMeasurement]=1 then 'Yes' else 'No' end ,UseAdjustedMeasurementFlag=cast([UseAdjustedMeasurement] as bit)
 from blobFull.SAB_ConsAttribute t
GO
